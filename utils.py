...
def get_output_directory(args):
    ...
    output_directory = os.path.join('results',
                                    ...
                                    format(args.data, args.depth_weight_head_type, args.num_samples, args.modality, \
                                           args.arch, args.criterion, args.depth_divider, args.lr, args.lrs,
                                           args.batch_size, \
                                           pretrain_text))
...
def merge_into_row_with_gt(input, depth_input, depth_target, depth_pred, normal_target=None, normal_pred=None,
                           valid_mask=None, new_prediction=None):
    rgb = 255 * np.transpose(np.squeeze(input.cpu().numpy()), (1, 2, 0))  # H, W, C
    ...
    d_min = min(np.min(depth_input_cpu[mask]), np.min(depth_target_cpu[target_depth_mask]),
                np.min(depth_pred_cpu[target_depth_mask]))
    ...
    img_merge = np.hstack(
        [rgb, depth_input_col, normal_target_cpu, normal_pred_cpu, depth_target_col, depth_pred_col, hist, diff_col_abs,
         diff_col_rel, diff_col_rel01, diff_col_rel01_pred, new_depth_pred_col])
...
def write_minmax(size_image, dmin, dmax, cmin, cmax):
    ...
    ax.text(0.0, 0.0, "dmin:{0:.4f}\ndmax:{1:.4f}\ncmin:{2:.4f}\ncmax:{3:.4f}".format(dmin, dmax, cmin, cmax),
            fontsize=45)
    ax.axis('off')
